...
import { TeamPageSkeleton } from "@/components/shared/Skeletons";
...
export default function TeamPage() {
  ...
  const isCaptain = team.members.some(m =>
    m.userId === profile?.id && ["CAPTAIN","captain","ADMIN","admin"].includes(m.role)
  );

  const inviteLink = typeof window !== "undefined"
    ? `${window.location.origin}/join/${team.inviteCode}` : `/join/${team.inviteCode}`;

  const shareInviteLink = () => {
    const text = `Join ${team.name} on Korum! 🏏\n\nWe use Korum to manage matches — RSVP, pay fees, and get your spot confirmed in seconds.\n\nJoin the squad 👉 ${inviteLink}`;
    if (navigator.share) void navigator.share({ title: `Join ${team.name}`, text, url: inviteLink });
    else window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, "_blank");
    track("team_invite_shared", { teamId: params.id });
  };

  const copyInviteLink = async () => {
    try { await navigator.clipboard.writeText(inviteLink); }
    catch { const el = document.createElement("textarea"); el.value = inviteLink; document.body.appendChild(el); el.select(); document.execCommand("copy"); document.body.removeChild(el); }
    setCopied(true);
    track("team_invite_copied", { teamId: params.id });
    setTimeout(() => setCopied(false), 2000);
  };

  return (
    <ErrorBoundary section="team page">
      <main>
          ...
          <section className="card card-pad animate-in" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button onClick={() => void copyInviteLink()}
                style={{ flexShrink: 0, padding: "10px 14px", border: "1.5px solid", borderColor: copied ? "var(--green-border)" : "var(--blue-border)", borderRadius: "var(--r-md)", background: copied ? "var(--green-soft)" : "var(--blue-soft)", color: copied ? "var(--green)" : "var(--blue)", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 13, cursor: "pointer", transition: "all 200ms", minWidth: 72 }}>
                {copied ? "✓ Copied!" : "Copy"}
              </button>
            </div>
          ...
          {isCaptain && (
            <section className="panel animate-in" style={{ display: "grid", gap: "0.75rem" }}>
              ...
              <div className="cluster">
                <Link href={`/team/${params.id}/availability`}><Button variant="secondary">📋 Check Availability</Button></Link>
                <Link href={`/create/match?teamId=${params.id}`}><Button>⚽ Create Match</Button></Link>
                <Link href={`/team/${params.id}/procurement`}><Button variant="secondary">🛒 Squad Shopping</Button></Link>
              </div>
            </section>
          )}
