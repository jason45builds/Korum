            ...
            {answered.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                {answered.map(item => {
                  ...
                  const cfg = STATUS_CONFIG[resp as keyof typeof STATUS_CONFIG];
                  return (
          ...
          <>
            <div className="card card-pad" style={{ background: "var(--blue-soft)", borderColor: "var(--blue-border)" }}>
              ...
              <p style={{ margin: "4px 0 0", fontSize: 13, color: "#1e40af" }}>
                Captains see your availability when they check who can play.
              </p>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 6 }}>
                {next14.map(d => {
                  ...
                  const entryCfg = entry ? STATUS_CONFIG[entry.status] : null;
                  return (
                    <button key={d} onClick={() => setSelDate(d)}
                      style={{
                        ...
                        background: isSelected ? "var(--blue)" : entryCfg ? entryCfg.soft : "var(--surface)",
                        borderColor: isSelected ? "var(--blue)" : entryCfg ? entryCfg.border : "var(--line)",
                      ...
                      {entryCfg && !isSelected && <span style={{ fontSize: 8 }}>{entryCfg.emoji}</span>}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="card card-pad animate-pop" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  {STATUS_KEYS.map(val => {
                    ...
                    return (
                      <button key={val}
                        ...
                        {cfg.label}
                        {selStatus === val && <span style={{ marginLeft: "auto", fontSize: 16 }}>✓</span>}
                      </button>
                    );
                  })}
                </div>
                ...
                <div className="field">
                  ...
                  {teams.map(t => (
                    <button key={t.id} onClick={() => toggleTeam(t.id)}
                      ...
                      {t.name}
                      <span style={{ fontSize: 11, color: "var(--text-4)", fontWeight: 400, marginLeft: 4 }}>{t.sport}</span>
              ...
              <button
                ...
                disabled={saving}
                style={{ width: "100%", minHeight: 52, border: "none", borderRadius: "var(--r-lg)", background: STATUS_CONFIG[selStatus].bg, color: "#fff", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 16, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
                <span style={{ fontSize: 20 }}>{STATUS_CONFIG[selStatus].emoji}</span>
                {saving ? "Saving…" : `Mark as ${STATUS_CONFIG[selStatus].label}`}
              </button>
            </div>
