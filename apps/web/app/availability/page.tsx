...
const fmtDate    = (d: string) => { try { return new Date(d + "T00:00:00").toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" }); } catch { return d; } };
const fmtShort   = (d: string) => { try { return new Date(d + "T00:00:00").toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short" }); } catch { return d; } };
...
export default function AvailabilityPage() {
  ...
  useEffect(() => { if (isAuthenticated) void loadInit(); }, [isAuthenticated]);
  ...
  const loadInit = async () => {
    ...
    try {
      ...
      // Issue #25: normalise role to uppercase before comparing — DB may store lowercase
      const normalise = (r: unknown) => String(r ?? "").toUpperCase();
      const caps = teamRes.teams.filter(t =>
        t.members?.some(m => m.userId === profile?.id && ["CAPTAIN","ADMIN"].includes(normalise(m.role)))
      );
  ...
  useEffect(() => { if (!capTeam || mode !== "captain") return; void loadCapCalendar(); }, [capTeam, mode]);
  useEffect(() => { if (!capTeam || mode !== "captain") return; void loadCapDetail();   }, [capTeam, capDate, mode]);

  const loadCapCalendar = async () => {
    try {
      const res  = await fetch("/api/team/availability", {
        method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin",
        body: JSON.stringify({ teamId: capTeam, startDate: today, endDate: next14[next14.length - 1] }),
      });
  ...
  const saveAvailability = async () => {
    ...
    try {
      await fetch("/api/availability-check", {
        method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin",
        body: JSON.stringify({ playerInitiated: true, matchDate: selDate, matchTime: selTime || null, status: selStatus, teamIds: selTeams.length > 0 ? selTeams : null }),
      });
      setMyEntries(e => [{ date: selDate, time: selTime, teamIds: selTeams, status: selStatus }, ...e.filter(x => x.date !== selDate)]);
      setSaveMsg({ text: "Saved! Your captain can now see your availability.", ok: true });
      setTimeout(() => setSaveMsg(null), 3500);
    } catch { setSaveMsg({ text: "Could not save — try again", ok: false }); }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 5 }}>
          {next14.map(d => {
            ...
            const av  = cnt?.available ?? 0;
            const tot = cnt?.totalMembers ?? 0;
            const pct = tot > 0 ? av / tot : 0;
            const isSel   = capDate === d;
            const isToday = d === today;
            let bg     = "var(--surface-2)", border = "var(--line)";
            if (av > 0 && pct >= 0.5) { bg = "var(--green-soft)"; border = "var(--green-border)"; }
            else if (av > 0)           { bg = "var(--amber-soft)"; border = "var(--amber-border)"; }
            if (isSel) { bg = "var(--blue)"; border = "var(--blue)"; }
            return (
              <button key={d} onClick={() => setCapDate(d)}
                ...
                <span style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em", color: isSel ? "#fff" : "var(--text-4)" }}>{fmtWeekday(d)}</span>
                <span style={{ fontSize: 15, fontFamily: "var(--font-display)", fontWeight: 900, color: isSel ? "#fff" : "var(--text)" }}>{fmtDayNum(d)}</span>
                {isToday && <span style={{ width: 4, height: 4, borderRadius: "50%", background: isSel ? "#fff" : "var(--blue)" }} />}
                {av > 0 && !isSel && <span style={{ fontSize: 9, fontWeight: 700, color: pct >= 0.5 ? "#166534" : "#92400e" }}>{av}</span>}
              </button>
            );
          })}
        </div>
      ...
      <div className="card" style={{ overflow: "hidden" }}>
        <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--line)", background: "var(--surface-2)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ margin: 0, fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 15 }}>{fmtDate(capDate)}</p>
          {capAv.length > 0 && (
            <div style={{ display: "flex", gap: 10 }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: "#166534" }}>✅ {capAv.filter(p => p.status === "AVAILABLE").length}</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: "#92400e" }}>🤔 {capAv.filter(p => p.status === "MAYBE").length}</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: "var(--text-4)" }}>⏳ {capAv.filter(p => p.status === "NO_RESPONSE").length}</span>
            </div>
          )}
        </div>
        {capLoading ? (
    ...
    <>
      <div className="tab-bar">
        <button className={`tab ${tab === "requests" ? "tab--active" : ""}`} onClick={() => setTab("requests")}>
          Requests {unanswered.length > 0 && <span style={{ background: "var(--red)", color: "#fff", borderRadius: "var(--r-full)", padding: "0 6px", fontSize: 10, marginLeft: 4 }}>{unanswered.length}</span>}
        </button>
        <button className={`tab ${tab === "my-schedule" ? "tab--active" : ""}`} onClick={() => setTab("my-schedule")}>My Schedule</button>
      </div>
            ...
            return (
              <div key={item.id} className="card animate-in" style={{ overflow: "hidden" }}>
                ...
                <div className="card-pad" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div>
                    <p style={{ margin: 0, fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 17 }}>{fmtDate(chk.match_date)}</p>
                    ...
                    {chk.note && <div style={{ marginTop: 10, padding: "8px 12px", background: "var(--blue-soft)", borderRadius: "var(--r-sm)", border: "1px solid var(--blue-border)" }}><p style={{ margin: 0, fontSize: 13, color: "var(--blue)", fontStyle: "italic" }}>&ldquo;{chk.note}&rdquo;</p></div>}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {STATUS_KEYS.map(val => {
                      ...
                      return (
                        <button key={val} disabled={!!responding || expired} onClick={() => void respond(chk.id, val)}
      ...
      {tab === "my-schedule" && (
        <>
          <div style={{ padding: "12px 14px", background: "var(--blue-soft)", border: "1px solid var(--blue-border)", borderRadius: "var(--r-md)" }}>
            <p style={{ margin: 0, fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 14, color: "var(--blue)" }}>Mark your availability proactively</p>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#1e40af" }}>Your captain sees this when planning matches.</p>
          </div>
          <div>
              ...
              {next14.map(d => {
                ...
                return (
                  <button key={d} onClick={() => setSelDate(d)}
                    style={{ padding: "8px 4px", border: "1.5px solid", borderRadius: "var(--r-md)", cursor: "pointer", fontFamily: "var(--font-display)", fontWeight: 700, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: isSel ? "var(--blue)" : cfg ? cfg.soft : "var(--surface)", borderColor: isSel ? "var(--blue)" : cfg ? cfg.border : "var(--line)", color: isSel ? "#fff" : "var(--text)", transition: "all 120ms" }}>
                    ...
                    {isToday && <span style={{ width: 4, height: 4, borderRadius: "50%", background: isSel ? "#fff" : "var(--blue)" }} />}
                    {cfg && !isSel && <span style={{ fontSize: 8 }}>{cfg.emoji}</span>}
                  </button>
                );
              })}
            </div>
          ...
          <div className="card card-pad" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <p style={{ margin: 0, fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 16 }}>{fmtDate(selDate)}</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {STATUS_KEYS.map(val => {
                const cfg = STATUS_CFG[val];
                return (
            ...
            {saveMsg && <p style={{ margin: 0, padding: "10px 14px", borderRadius: "var(--r-md)", fontSize: 13, fontWeight: 600, background: saveMsg.ok ? "var(--green-soft)" : "var(--red-soft)", color: saveMsg.ok ? "#166534" : "#991b1b" }}>{saveMsg.text}</p>}
              ...
              {myEntries.map((e, i) => {
                ...
                return (
                  <div key={i} className="card" style={{ padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <div>
                      <p style={{ margin: 0, fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 14 }}>{fmtShort(e.date)}{e.time ? ` · ${e.time}` : ""}</p>
                      <p className="t-caption" style={{ marginTop: 2 }}>{e.teamIds.length === 0 ? "All teams" : `${e.teamIds.length} team${e.teamIds.length > 1 ? "s" : ""}`}</p>
                    </div>
                    <span style={{ padding: "4px 12px", borderRadius: "var(--r-full)", background: cfg.soft, border: `1px solid ${cfg.border}`, color: cfg.text, fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 12 }}>{cfg.emoji} {cfg.label}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ display: "flex", background: "var(--surface-2)", borderRadius: "var(--r-full)", padding: 3, gap: 2, border: "1px solid var(--line)" }}>
              {(["player","captain"] as const).map(m => (
                <button key={m} onClick={() => setMode(m)}
                  style={{ padding: "6px 14px", borderRadius: "var(--r-full)", border: "none", background: mode === m ? "#fff" : "transparent", color: mode === m ? "var(--blue)" : "var(--text-3)", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 12, cursor: "pointer", boxShadow: mode === m ? "0 1px 4px rgba(0,0,0,0.1)" : "none", transition: "all 140ms", textTransform: "capitalize" }}>
                  {m}
                </button>
              ))}
            </div>
