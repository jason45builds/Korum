...
export default function DashboardPage() {
  ...
  if (loading && dashboardMatches.length === 0) {
    return <main><Loader label="Loading dashboard…" /></main>;
  }

  const handleCreateTeam = async () => {
    if (!teamDraft.name.trim()) {
      setCreateMsg({ text: "Enter a team name.", error: true });
      return;
    }
    ...
    try {
      ...
      setCreateMsg({ text: error instanceof Error ? error.message : "Could not create team.", error: true });
    } finally {
  ...
  const handleJoinTeam = async () => {
    if (!inviteCode.trim()) {
      setJoinMsg({ text: "Enter an invite code.", error: true });
      return;
    }
    setJoiningTeam(true);
    setJoinMsg(null);
    try {
      ...
      setJoinMsg({ text: error instanceof Error ? error.message : "Could not join team.", error: true });
    } finally {
          ...
          <Card eyebrow="Teams" title="Create a team">
            <div className="form-grid">
              <label className="label">
                ...
                <input
                  className="input"
                  placeholder="FC Sunday Warriors"
