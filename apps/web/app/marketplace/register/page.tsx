...
function RegisterContent() {
  ...
  // Vendor form
  const [vForm, setVForm] = useState({ name: "", category: "Kit", description: "", city: "", contactPhone: "", contactEmail: "", website: "", priceNote: "", sports: [] as string[] });
  // Ground form
  const [gForm, setGForm] = useState({ name: "", address: "", city: "", state: "", pricePerHour: "", capacity: "", surface: "Turf", amenities: [] as string[], contactPhone: "", contactEmail: "", sports: [] as string[] });

  const toggleSport = (sport: string, form: { sports: string[] }, setForm: (fn: (f: typeof form) => typeof form) => void) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = type === "vendor"
        ? { ...vForm }
        : { ...gForm, pricePerHour: gForm.pricePerHour ? Number(gForm.pricePerHour) : null, capacity: gForm.capacity ? Number(gForm.capacity) : null, sport: gForm.sports };
      ...
      <div className="page">
        <div>
          ...
          <p className="t-caption" style={{ marginTop: 4, color: "var(--text-3)" }}>
            {type === "vendor" ? "Reach sports teams in your city. Kit, equipment, food, physio and more." : "Make your venue discoverable to local teams."}
          </p>
        </div>
            ...
            {type === "vendor" ? (
              <>
                <div className="field"><label className="field-label">Business name *</label>
                  <input className="input" value={vForm.name} onChange={e => setVForm(f => ({ ...f, name: e.target.value }))} placeholder="Sports Hub Chennai" /></div>
                ...
                <div className="field"><label className="field-label">City *</label>
                  <input className="input" value={vForm.city} onChange={e => setVForm(f => ({ ...f, city: e.target.value }))} placeholder="Chennai" /></div>
                <div className="field"><label className="field-label">Description</label>
                  <textarea className="input" style={{ minHeight: 80 }} value={vForm.description} onChange={e => setVForm(f => ({ ...f, description: e.target.value }))} placeholder="What you offer, turnaround time, delivery…" /></div>
                <div className="field"><label className="field-label">Pricing note</label>
                  <input className="input" value={vForm.priceNote} onChange={e => setVForm(f => ({ ...f, priceNote: e.target.value }))} placeholder="₹500/kit · Custom quote available" /></div>
                ...
                <div className="field"><label className="field-label">Sports you serve</label>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 4 }}>
                    {SPORT_OPTIONS.slice(0, 10).map(s => (
                      <button key={s} type="button" onClick={() => toggleSport(s, vForm, setVForm)}
                        style={{ padding: "5px 12px", borderRadius: "var(--r-full)", border: "1.5px solid", borderColor: vForm.sports.includes(s) ? "var(--blue)" : "var(--line)", background: vForm.sports.includes(s) ? "var(--blue-soft)" : "var(--surface)", color: vForm.sports.includes(s) ? "var(--blue)" : "var(--text-3)", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                        {s}
                      </button>
                    ))}
                  </div>
              ...
              <>
                <div className="field"><label className="field-label">Ground name *</label>
                  <input className="input" value={gForm.name} onChange={e => setGForm(f => ({ ...f, name: e.target.value }))} placeholder="Marina Cricket Ground" /></div>
                <div className="field"><label className="field-label">Address *</label>
                  <input className="input" value={gForm.address} onChange={e => setGForm(f => ({ ...f, address: e.target.value }))} placeholder="123 Beach Road" /></div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div className="field"><label className="field-label">City *</label>
                    <input className="input" value={gForm.city} onChange={e => setGForm(f => ({ ...f, city: e.target.value }))} placeholder="Chennai" /></div>
                  <div className="field"><label className="field-label">State</label>
                    <input className="input" value={gForm.state} onChange={e => setGForm(f => ({ ...f, state: e.target.value }))} placeholder="Tamil Nadu" /></div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div className="field"><label className="field-label">Price per hour (₹)</label>
                    <input className="input" type="number" value={gForm.pricePerHour} onChange={e => setGForm(f => ({ ...f, pricePerHour: e.target.value }))} placeholder="500" /></div>
                  <div className="field"><label className="field-label">Capacity</label>
                    <input className="input" type="number" value={gForm.capacity} onChange={e => setGForm(f => ({ ...f, capacity: e.target.value }))} placeholder="22" /></div>
                </div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 4 }}>
                    {AMENITIES_LIST.map(a => (
                      <button key={a} type="button" onClick={() => toggleAmenity(a)}
                        style={{ padding: "5px 12px", borderRadius: "var(--r-full)", border: "1.5px solid", borderColor: gForm.amenities.includes(a) ? "var(--green)" : "var(--line)", background: gForm.amenities.includes(a) ? "var(--green-soft)" : "var(--surface)", color: gForm.amenities.includes(a) ? "#166534" : "var(--text-3)", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                        {a}
                      </button>
                    ))}
                  </div>
                ...
                <div className="field"><label className="field-label">Sports</label>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 4 }}>
                    {SPORT_OPTIONS.slice(0, 10).map(s => (
                      <button key={s} type="button" onClick={() => toggleSport(s, gForm as { sports: string[] }, setGForm as unknown as (fn: (f: { sports: string[] }) => { sports: string[] }) => void)}
                        style={{ padding: "5px 12px", borderRadius: "var(--r-full)", border: "1.5px solid", borderColor: gForm.sports.includes(s) ? "var(--blue)" : "var(--line)", background: gForm.sports.includes(s) ? "var(--blue-soft)" : "var(--surface)", color: gForm.sports.includes(s) ? "var(--blue)" : "var(--text-3)", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                        {s}
                      </button>
                    ))}
                  </div>
